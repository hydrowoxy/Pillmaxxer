...
const ReminderModal = ({
  ...
  const logTimingInfo = () => {
    ...
    const withinThreshold =
      reminderLatencyMilliseconds <= THRESHOLD_IN_SECONDS * 1000

    const logEntry = {
      userId: reminder.userId,
      date: reminder.date,
      scheduledDose: reminder.scheduledDose.medications
    ...
    console.log(
      "Reminder Latency (Milliseconds):",
      logEntry.reminderLatencyMilliseconds
    )
    console.log(
      "Reminder Difference (Seconds):",
      logEntry.reminderLatencySeconds
    )
    if (withinThreshold) {
      console.log(
      ...
      <View style={styles.container}>
        ...
        >
          ...
          <Text style={styles.modalText}>
            Time: {reminder.scheduledDose.timeOfDay}
          </Text>
          <Text style={styles.modalText}>
            Medications:{" "}
            {reminder.scheduledDose.medications
              .map((med) => med.medicationName)
              .join(", ")}
          </Text>
